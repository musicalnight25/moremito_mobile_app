import 'package:get/get.dart';

class AppTranslations extends Translations {
  @override
  Map<String, Map<String, String>> get keys => {
        'en_US': {
          "MoreMito": "MoreMito",
          "Success 🎉": "Success 🎉",
          "Thanks for sharing via \$platform":
              "Thanks for sharing via \$platform",
          "Thanks for sharing via {platform}":
              "Thanks for sharing via @platform",
          "Required": "Required",
          "Please fill in all fields": "Please fill in all fields",
          "Enter Contact Details": "Enter Contact Details",
          "Recipient Name": "Recipient Name",
          "Phone Number": "Phone Number",
          "Phone number": "Phone number",
          "Email address": "Email address",
          "Share Via": "Share Via",
          "Share Now": "Share Now",
          "Share MoreMito Content": "Share MoreMito Content",
          "Generate Another Link": "Generate Another Link",
          "Step 1: Enter Recipient Name": "Step 1: Enter Recipient Name",
          "Step 2: Write Message": "Step 2: Write Message",
          "Mismatch": "Mismatch",
          "New password and confirm password do not match":
              "New password and confirm password do not match",
          "Weak Password": "Weak Password",
          "Password must be at least 6 characters":
              "Password must be at least 6 characters",
          "Success": "Success",
          "Password updated successfully": "Password updated successfully",
          "Error": "Error",
          "Something went wrong. Please try again.":
              "Something went wrong. Please try again.",
          "Permission Required": "Permission Required",
          "Please allow contacts permission":
              "Please allow contacts permission",
          "Login Failed": "Login Failed",
          "Please check your credentials.": "Please check your credentials.",
          "Profile updated successfully": "Profile updated successfully",
          "All notifications marked as read":
              "All notifications marked as read",
          "Shop Moremito": "Shop Moremito",
          "Incomplete Survey": "Incomplete Survey",
          "Please answer all questions before submitting.":
              "Please answer all questions before submitting.",
          "Your survey has been submitted.": "Your survey has been submitted.",
          "Submission failed, try again.": "Submission failed, try again.",
          "Unable to submit survey. Try again later.":
              "Unable to submit survey. Try again later.",
          "Ticket created successfully.": "Ticket created successfully.",
          "Ticket reopened": "Ticket reopened",
          "Welcome tag updated successfully":
              "Welcome tag updated successfully",
          "My Address Screen": "My Address Screen",
          "My Address": "My Address",
          "Link copied to clipboard": "Link copied to clipboard",
          "LINK URL": "LINK URL",
          "COPY": "COPY",
          "My Deep Links": "My Deep Links",
          "No Deep Links Generated": "No Deep Links Generated",
          "My Network Screen": "My Network Screen",
          "My Network": "My Network",
          "My Recurring Order Screen": "My Recurring Order Screen",
          "My Recurring Order": "My Recurring Order",
          "My Referral Orders Screen": "My Referral Orders Screen",
          "My Referral Orders": "My Referral Orders",
          "My Personal Referral's Orders": "My Personal Referral's Orders",
          "My Personal Referral’s Orders": "My Personal Referral’s Orders",
          "Date": "Date",
          "Order No.": "Order No.",
          "Rank": "Rank",
          "No rank history found": "No rank history found",
          "My Rank History": "My Rank History",
          "Current Rank": "Current Rank",
          "Highest Rank": "Highest Rank",
          "Penny for your thoughts?": "Penny for your thoughts?",
          "Close": "Close",
          "Username is required. Please enter your username.":
              "Username is required. Please enter your username.",
          "Password is required. Please enter your password.":
              "Password is required. Please enter your password.",
          "Remember me": "Remember me",
          "Enter username": "Enter username",
          "Enter password": "Enter password",
          "Username*": "Username*",
          "Password*": "Password*",
          "Login": "Login",
          "To continue, please take a moment to complete a brief survey. Your feedback helps us improve your experience!":
              "To continue, please take a moment to complete a brief survey. Your feedback helps us improve your experience!",
          "Start Survey": "Start Survey",
          "Answer Required": "Answer Required",
          "Please answer first.": "Please answer first.",
          "Select an answer first.": "Select an answer first.",
          "Survey": "Survey",
          "MoreMito Library": "MoreMito Library",
          "Browse audio, video, and document files.":
              "Browse audio, video, and document files.",
          "Search Audios, Videos & Docs": "Search Audios, Videos & Docs",
          "Files": "Files",
          "Categories": "Categories",
          "No Contacts Found": "No Contacts Found",
          "Search contacts...": "Search contacts...",
          "Select Contact": "Select Contact",
          "Audio": "Audio",
          "Playing audio file.": "Playing audio file.",
          "Please enter recipient name": "Please enter recipient name",
          "Failed to generate link": "Failed to generate link",
          "Share": "Share",
          "•  ": "•  ",
          "e.g. John Doe": "e.g. John Doe",
          "Write a message for the recipient":
              "Write a message for the recipient",
          "Enter recipient name": "Enter recipient name",
          "Message": "Message",
          "Select an Option": "Select an Option",
          "Please choose how you would like to share.":
              "Please choose how you would like to share.",
          "Choose from Contacts": "Choose from Contacts",
          "Enter Name Manually": "Enter Name Manually",
          "Generate Link To Share": "Generate Link To Share",
          "Generate a Link": "Generate a Link",
          "Enter the name to track the responses from this person with.":
              "Enter the name to track the responses from this person with.",
          "Click on \"Generate a Link\" button to create a link that you are going to send to this person.":
              "Click on \"Generate a Link\" button to create a link that you are going to send to this person.",
          "A personalized message has been created for the recipient.":
              "A personalized message has been created for the recipient.",
          "You can review and edit the message if needed.":
              "You can review and edit the message if needed.",
          "Click Share to send the message to the recipient.":
              "Click Share to send the message to the recipient.",
          "Hey {name}, {sender} has shared some information with you. Visit the link to view it. {url}":
              "Hey @name, @sender has shared some information with you. Visit the link to view it. @url",
          "Hey {name}, {sender} has invited you to the {title}. Visit the link to see more. {url}":
              "Hey @name, @sender has invited you to the @title. Visit the link to see more. @url",
          "Hey {name}, {sender} has invited you to the MoreMito health and wellness call. Visit the link to see more. {url}":
              "Hey @name, @sender has invited you to the MoreMito health and wellness call. Visit the link to see more. @url",
          "Subcategories": "Subcategories",
          "No sent history found": "No sent history found",
          "MoreMito Cash Transfer History": "MoreMito Cash Transfer History",
          "Below is the history of the MoreMito Cash you have transferred.":
              "Below is the history of the MoreMito Cash you have transferred.",
          "MoreMito Cash Transferred By You":
              "MoreMito Cash Transferred By You",
          "Total Sent: ": "Total Sent: ",
          "MoreMito Cash Sent To Others": "MoreMito Cash Sent To Others",
          "Date Transferred": "Date Transferred",
          "Sent To": "Sent To",
          "Amount": "Amount",
          "No transfer history found": "No transfer history found",
          "Below is the history of the MoreMito Cash you have received.":
              "Below is the history of the MoreMito Cash you have received.",
          "MoreMito Cash Transferred To You":
              "MoreMito Cash Transferred To You",
          "Total Received: ": "Total Received: ",
          "Transfer History": "Transfer History",
          "Date Received": "Date Received",
          "Sent By": "Sent By",
          "No payout history found": "No payout history found",
          "Requested Commission History": "Requested Commission History",
          "Total Approved Amount": "Total Approved Amount",
          "Commission Payout History": "Commission Payout History",
          "Transaction Id": "Transaction Id",
          "Payment Method": "Payment Method",
          "Payment Status": "Payment Status",
          "Description": "Description",
          "Go": "Go",
          "Total Amount": "Total Amount",
          "Refunded": "Refunded",
          "No records found": "No records found",
          "Search by Order No.": "Search by Order No.",
          "Compensation Spent On Orders": "Compensation Spent On Orders",
          "No orders found": "No orders found",
          "Order Details": "Order Details",
          "Commission Type Summary": "Commission Type Summary",
          "Commissions Details": "Commissions Details",
          "No compensation history found": "No compensation history found",
          "Yearly Details": "Yearly Details",
          "Total Compensation": "Total Compensation",
          "My Compensation": "My Compensation",
          "MoreMito Cash": "MoreMito Cash",
          "MoreMito Commission": "MoreMito Commission",
          "Total Compensation Earned": "Total Compensation Earned",
          "Average Order Amount": "Average Order Amount",
          "Customer Count": "Customer Count",
          "Avg. Earned / Customer": "Avg. Earned / Customer",
          "View All Logs": "View All Logs",
          "Daily Compensation Log": "Daily Compensation Log",
          "No details found": "No details found",
          "\$orderId": "\$orderId",
          "Commission processing date": "Commission processing date",
          "No data available for \$year": "No data available for \$year",
          "No data available for {year}": "No data available for @year",
          "\$year Details": "\$year Details",
          "{year} Details": "@year Details",
          "No content available": "No content available",
          "Announcement Details": "Announcement Details",
          "Call Details": "Call Details",
          "Welcome": "Welcome",
          "\${user.name} (\${user.userName})":
              "\${user.name} (\${user.userName})",
          "Call Announcement": "Call Announcement",
          "Subject: \${call.subject ?? ": "Subject: \${call.subject ?? ",
          "Click to View": "Click to View",
          "Announcements": "Announcements",
          "BETA VERSION": "BETA VERSION",
          "v1.0.0": "v1.0.0",
          "This app is currently in Beta, which means some features may be incomplete, under testing, or subject to change. You may experience occasional issues or variations. We appreciate your feedback as we work to improve the app.":
              "This app is currently in Beta, which means some features may be incomplete, under testing, or subject to change. You may experience occasional issues or variations. We appreciate your feedback as we work to improve the app.",
          "Dashboard": "Dashboard",
          "Confirm Exit": "Confirm Exit",
          "Home": "Home",
          "Share Info": "Share Info",
          "Notification": "Notification",
          "Setting": "Setting",
          "No interactions yet": "No interactions yet",
          "Notes saved successfully": "Notes saved successfully",
          "\$label:": "\$label:",
          "{label}:": "@label:",
          "Enter internal notes": "Enter internal notes",
          "Lead Details": "Lead Details",
          "My Leads": "My Leads",
          "Contacted": "Contacted",
          "View Details": "View Details",
          "My Mito Info Shared Links Activity Tracking":
              "My Mito Info Shared Links Activity Tracking",
          "Track and monitor all your shared links activity.\nView recipient activity and interaction details.":
              "Track and monitor all your shared links activity.\nView recipient activity and interaction details.",
          "Share\nActivity": "Share\nActivity",
          "Shared\nby Me": "Shared\nby Me",
          "Shared\nwith Me": "Shared\nwith Me",
          "{name}'s Activity": "@name's Activity",
          "{nameWithHandle}\nShared Links Activity Tracking":
              "@nameWithHandle\nShared Links Activity Tracking",
          "View shared links activity report shared with you.\nView recipient activity and interaction details.":
              "View shared links activity report shared with you.\nView recipient activity and interaction details.",
          "No activity data available.\nPull down to refresh.":
              "No activity data available.\nPull down to refresh.",
          "Last 72 Hours": "Last 72 Hours",
          "Last 7 Days": "Last 7 Days",
          "Last 2 weeks": "Last 2 weeks",
          "Last 3 weeks": "Last 3 weeks",
          "Last 4 weeks": "Last 4 weeks",
          "Lifetime": "Lifetime",
          "People Health\nProducts": "People Health\nProducts",
          "Pet Health\nProducts": "Pet Health\nProducts",
          "Buy Now": "Buy Now",
          "All": "All",
          "Audios, Videos & Docs": "Audios, Videos & Docs",
          "Flyers": "Flyers",
          "SMS requested info ": "SMS requested info ",
          "Total Activities : \${item.totalInteractions ?? 0}":
              "Total Activities : \${item.totalInteractions ?? 0}",
          "Total Activities : {total}": "Total Activities : @total",
          "Search recipient": "Search recipient",
          "Shared Flyers": "Shared Flyers",
          "Who :": "Who :",
          "What :": "What :",
          "When :": "When :",
          "Last Activity :": "Last Activity :",
          "Activity Tracking Summary (all links)":
              "Activity Tracking Summary (all links)",
          "These cards show total recipients and total activity ":
              "These cards show total recipients and total activity ",
          "Retry": "Retry",
          "\${widget.userName}": "\${widget.userName}",
          "{name} ({username})": "@name (@username)",
          "Remove Share": "Remove Share",
          "Cancel": "Cancel",
          "Remove": "Remove",
          "Users I Have Shared Reports With":
              "Users I Have Shared Reports With",
          "Manage users who can see your shared link activity report.":
              "Manage users who can see your shared link activity report.",
          "You haven": "You haven",
          "My Shared Reports": "My Shared Reports",
          "Decline Report": "Decline Report",
          "Decline report shared by ": "Decline report shared by ",
          "Decline": "Decline",
          "View": "View",
          "Activities Shared With Me": "Activities Shared With Me",
          "Reports shared with you by other users.":
              "Reports shared with you by other users.",
          "No activities shared with you yet.":
              "No activities shared with you yet.",
          "Reason (optional)": "Reason (optional)",
          "Shared With Me": "Shared With Me",
          "Share Your Activity": "Share Your Activity",
          "Search": "Search",
          "Enter a username and tap Search.":
              "Enter a username and tap Search.",
          "\${results.length} result(s)": "\${results.length} result(s)",
          "{count} result(s)": "@count result(s)",
          "@\${user.username ?? ": "@\${user.username ?? ",
          "Choose": "Choose",
          "Share Report": "Share Report",
          "Review the selected user and optionally add a note before sharing.":
              "Review the selected user and optionally add a note before sharing.",
          "Selected User Details": "Selected User Details",
          "Note (Optional):": "Note (Optional):",
          "Different User": "Different User",
          "Enter a note (max 500 characters)":
              "Enter a note (max 500 characters)",
          "Search by Username": "Search by Username",
          "Allow Others to Request MoreMito Info from Me":
              "Allow Others to Request MoreMito Info from Me",
          "Notes": "Notes",
          "TMRIS Leads": "TMRIS Leads",
          "-": "-",
          "TMRIS Lead Details": "TMRIS Lead Details",
          "Order Details for Order No. ": "Order Details for Order No. ",
          "\${address.address1}, \${address.city}, \${address.stateName}, \${address.countryName} - \${address.zip}":
              "\${address.address1}, \${address.city}, \${address.stateName}, \${address.countryName} - \${address.zip}",
          "Product(s)": "Product(s)",
          "Quantity: \${p.quantity}": "Quantity: \${p.quantity}",
          "Quantity: {quantity}": "Quantity: @quantity",
          "Notification Details": "Notification Details",
          "Notifications": "Notifications",
          "Mark all read": "Mark all read",
          "Push Notifications": "Push Notifications",
          "Manage how you receive alerts and updates":
              "Manage how you receive alerts and updates",
          "Enable/Disable app push notifications":
              "Enable/Disable app push notifications",
          "All Notifications": "All Notifications",
          "Shared Link Notifications": "Shared Link Notifications",
          "Messages about promotions and information sent by us":
              "Messages about promotions and information sent by us",
          "Announcement Notifications": "Announcement Notifications",
          "Important updates and reminders": "Important updates and reminders",
          "Reset": "Reset",
          "Downline Orders": "Downline Orders",
          "No order details found": "No order details found",
          "Order #\${data.myOrders.orderId}":
              "Order #\${data.myOrders.orderId}",
          "Order #{orderId}": "Order #@orderId",
          "Products": "Products",
          "Downline Order Details": "Downline Order Details",
          "Order Number : \${order.orderId}":
              "Order Number : \${order.orderId}",
          "Order Number : {orderId}": "Order Number : @orderId",
          "Shipping Method": "Shipping Method",
          "Tracking ID": "Tracking ID",
          "Send/View messages with support team":
              "Send/View messages with support team",
          "My Order": "My Order",
          "Orders": "Orders",
          "Referral & Downline Orders": "Referral & Downline Orders",
          "Billing Address": "Billing Address",
          "Order Summary": "Order Summary",
          "Order Number - \${_value(info?.customOrderNumber)}":
              "Order Number - \${_value(info?.customOrderNumber)}",
          "Order Number - {orderNumber}": "Order Number - @orderNumber",
          "Order Date: \$date (UTC)": "Order Date: \$date (UTC)",
          "Order Date: {date} (UTC)": "Order Date: @date (UTC)",
          "Shipping Address": "Shipping Address",
          "Please select Country and State": "Please select Country and State",
          "Set as primary address": "Set as primary address",
          "Select \$label": "Select \$label",
          "Select {label}": "Select @label",
          "Contact Person": "Contact Person",
          "Address": "Address",
          "Country": "Country",
          "State": "State",
          "Create new password": "Create new password",
          "Your new password must be different from previously used passwords.":
              "Your new password must be different from previously used passwords.",
          "Please Enter \$label": "Please Enter \$label",
          "Please Enter {label}": "Please Enter @label",
          "Change Password": "Change Password",
          "Update Password": "Update Password",
          "Current Password": "Current Password",
          "New Password": "New Password",
          "Confirm Password": "Confirm Password",
          "Note: Changing your address here does not update existing recurring orders. Please update those manually in the Orders tab.":
              "Note: Changing your address here does not update existing recurring orders. Please update those manually in the Orders tab.",
          "\${address.firstName} \${address.lastName}":
              "\${address.firstName} \${address.lastName}",
          "Default": "Default",
          "No addresses yet": "No addresses yet",
          "Add your delivery address to get started":
              "Add your delivery address to get started",
          "Manage Addresses": "Manage Addresses",
          "Edit": "Edit",
          "Set as Default": "Set as Default",
          "Add New Address": "Add New Address",
          "Note: Changing your address here does not change your address on already created recurring orders.":
              "Note: Changing your address here does not change your address on already created recurring orders.",
          "No addresses found.": "No addresses found.",
          "My Profile": "My Profile",
          "Account Information": "Account Information",
          "Username": "Username",
          "SMS Code": "SMS Code",
          "First Name": "First Name",
          "Last Name": "Last Name",
          "Phone": "Phone",
          "WhatsApp Phone": "WhatsApp Phone",
          "Membership Type": "Membership Type",
          "Join Date": "Join Date",
          "City": "City",
          "Zip": "Zip",
          "Name": "Name",
          "Update Profile": "Update Profile",
          "Manage your addresses": "Manage your addresses",
          "Edit address": "Edit address",
          "Your Website Header Display": "Your Website Header Display",
          "Loading...": "Loading...",
          "User Role": "User Role",
          "Change Role": "Change Role",
          "Use aliases (my Welcome Tag info) for name, email, and phone number":
              "Use aliases (my Welcome Tag info) for name, email, and phone number",
          "Save Changes": "Save Changes",
          "Grief Relief Zoom Call Screen": "Grief Relief Zoom Call Screen",
          "Grief Relief Zoom Call": "Grief Relief Zoom Call",
          "My Info": "My Info",
          "My Account Settings": "My Account Settings",
          "My Personals": "My Personals",
          "My Support Network": "My Support Network",
          "Email Notification Settings": "Email Notification Settings",
          "Text Notification Settings": "Text Notification Settings",
          "User Survey": "User Survey",
          "My Tree View": "My Tree View",
          "My Star Tree View": "My Star Tree View",
          "Search My Network": "Search My Network",
          "Shop MoreMito Products": "Shop MoreMito Products",
          "My Orders": "My Orders",
          "My Personal Referral": "My Personal Referral",
          "Create A New Autoship Order": "Create A New Autoship Order",
          "Recurring Orders": "Recurring Orders",
          "Payment Settings": "Payment Settings",
          "Compensation": "Compensation",
          "My Compensations": "My Compensations",
          "My Daily Compensation Log": "My Daily Compensation Log",
          "My Compensation History": "My Compensation History",
          "Request Payouts And Make Transfers":
              "Request Payouts And Make Transfers",
          "Request A Payout": "Request A Payout",
          "History Of Compensation Spent": "History Of Compensation Spent",
          "My Currently Available Balance": "My Currently Available Balance",
          "Manage My Payout Methods": "Manage My Payout Methods",
          "The Compensation Plan": "The Compensation Plan",
          "My Rank Management": "My Rank Management",
          "Rank Management Report": "Rank Management Report",
          "Resources": "Resources",
          "Direct Links": "Direct Links",
          "Grief Relief Zoom Call 7-14-24": "Grief Relief Zoom Call 7-14-24",
          "Health & Wellness Call Replays": "Health & Wellness Call Replays",
          "Share MoreMito Info": "Share MoreMito Info",
          "Customize And Share My Flyers": "Customize And Share My Flyers",
          "My Flyers": "My Flyers",
          "Share Audios, Videos & Docs Files":
              "Share Audios, Videos & Docs Files",
          "Allow Others To Request MoreMito Info From Me":
              "Allow Others To Request MoreMito Info From Me",
          "See My Generated Leads": "See My Generated Leads",
          "See & Track Activity From My Shared Links":
              "See & Track Activity From My Shared Links",
          "Support": "Support",
          "Support Ticket List": "Support Ticket List",
          "Create Support Ticket": "Create Support Ticket",
          "Order Comments": "Order Comments",
          "Transfer MoreMito Cash": "Transfer MoreMito Cash",
          "Menu": "Menu",
          "Mitochondria Story Screen": "Mitochondria Story Screen",
          "Mitochondria Story": "Mitochondria Story",
          "Settings": "Settings",
          "Account": "Account",
          "Actions": "Actions",
          "Logout": "Logout",
          "Confirm Logout": "Confirm Logout",
          "Testimonials Screen": "Testimonials Screen",
          "Testimonials": "Testimonials",
          "Exciting features are on the way! Stay tuned for updates and new enhancements.":
              "Exciting features are on the way! Stay tuned for updates and new enhancements.",
          "Back to Home": "Back to Home",
          "Warning": "Warning",
          "Please fill all fields": "Please fill all fields",
          "Enter subject": "Enter subject",
          "Select": "Select",
          "Describe your issue (order no., username, etc.)":
              "Describe your issue (order no., username, etc.)",
          "Ticket Subject:": "Ticket Subject:",
          "Ticket Related To:": "Ticket Related To:",
          "Ticket Priority:": "Ticket Priority:",
          "Ticket Description:": "Ticket Description:",
          "Create Ticket": "Create Ticket",
          "#\${ticket.ticketId}": "#\${ticket.ticketId}",
          "Priority": "Priority",
          "Subject": "Subject",
          "Re-open Ticket": "Re-open Ticket",
          "Support Tickets": "Support Tickets",
          "Enter comment": "Enter comment",
          "Previous Comments": "Previous Comments",
          "No comments yet": "No comments yet",
          "Add Comment": "Add Comment",
          "\${c.firstName ?? ": "\${c.firstName ?? ",
          "Write your comment...": "Write your comment...",
          "Ticket #\${widget.ticketId}": "Ticket #\${widget.ticketId}",
          "Ticket #{ticketId}": "Ticket #@ticketId",
          "Submit Comment": "Submit Comment",
          "Invalid username or password.": "Invalid username or password.",
          "Access Denied!": "Access Denied!",
          "Session expired. Please log in again.":
              "Session expired. Please log in again.",
          "This update is mandatory": "This update is mandatory",
          "Update Now": "Update Now",
          "Connection Error: \$e": "Connection Error: \$e",
          "Connection Error: {error}": "Connection Error: @error",
          "You": "You",
          "No Share 😕": "No Share 😕",
          "Looks like the share wasn’t completed. No worries! You can try again whenever you":
              "Looks like the share wasn’t completed. No worries! You can try again whenever you",
          "Looks like the share wasn’t completed. No worries! You can try again whenever you're ready. We appreciate you! ❤️":
              "Looks like the share wasn’t completed. No worries! You can try again whenever you're ready. We appreciate you! ❤️",
          "You're amazing! 🌟 Thank you for sharing on \$platform. We truly appreciate your support! 🙌😊":
              "You're amazing! 🌟 Thank you for sharing on \$platform. We truly appreciate your support! 🙌😊",
          "You're amazing! 🌟 Thank you for sharing on {platform}. We truly appreciate your support! 🙌😊":
              "You're amazing! 🌟 Thank you for sharing on @platform. We truly appreciate your support! 🙌😊",
          "WhatsApp": "WhatsApp",
          "Email": "Email",
          "More": "More",
          "Test": "Test",
          "⚠️ iOS token fetch error: \$e": "⚠️ iOS token fetch error: \$e",
          "⚠️ iOS token fetch error: {error}":
              "⚠️ iOS token fetch error: @error",
          "Check your internet connection.": "Check your internet connection.",
          "Try again in a few moments.": "Try again in a few moments.",
          "Please grant \$permission permission in the settings.":
              "Please grant \$permission permission in the settings.",
          "Please grant {permission} permission in the settings.":
              "Please grant @permission permission in the settings.",
          "Open Settings": "Open Settings",
          "Lost Connection": "Lost Connection",
          "Preview Document": "Preview Document",
          "Open in Browser": "Open in Browser",
          "Token copied to clipboard!": "Token copied to clipboard!",
          "FCM Token": "FCM Token",
          "Your Device Token:": "Your Device Token:",
          "Copy Token": "Copy Token",
          "No internet connection": "No internet connection",
          "Please check your keyword or try again your browsing keyword":
              "Please check your keyword or try again your browsing keyword",
          "Check your network": "Check your network",
          "Try again": "Try again",
          "Oops! No \${widget.title ?? ": "Oops! No \${widget.title ?? ",
          "Loading": "Loading",
          "Preparing page...": "Preparing page...",
          "Video failed to load": "Video failed to load",
          "Error loading video": "Error loading video",
          "Language changed to English": "Language changed to English",
          "Language changed to Chinese": "Language changed to Chinese",
          "Activity": "Activity",
          "Address Line 1": "Address Line 1",
          "Address Line 2": "Address Line 2",
          "Amount Paid": "Amount Paid",
          "Average Amount": "Average Amount",
          "Avg Earned / Customer": "Avg Earned / Customer",
          "Call Me": "Call Me",
          "Commission Level": "Commission Level",
          "Commission Type": "Commission Type",
          "Compensation %": "Compensation %",
          "Compensation Amount": "Compensation Amount",
          "Compensation Type": "Compensation Type",
          "Contacted Date": "Contacted Date",
          "Customer": "Customer",
          "Email Address": "Email Address",
          "Gross Order Amount": "Gross Order Amount",
          "Interest": "Interest",
          "Item Total": "Item Total",
          "Level": "Level",
          "Order Amount": "Order Amount",
          "Order Count": "Order Count",
          "Order Date": "Order Date",
          "Order No": "Order No",
          "Order Total": "Order Total",
          "OrderId": "OrderId",
          "Payment": "Payment",
          "Price": "Price",
          "Product": "Product",
          "Qty": "Qty",
          "Quantity": "Quantity",
          "Recipients": "Recipients",
          "Running Total": "Running Total",
          "Sales Tax": "Sales Tax",
          "Shipping": "Shipping",
          "Shipping Amount": "Shipping Amount",
          "Shipping Fee": "Shipping Fee",
          "Status": "Status",
          "Street Address": "Street Address",
          "Sub Total": "Sub Total",
          "Submitted": "Submitted",
          "Tax": "Tax",
          "Total": "Total",
          "Total Earned": "Total Earned",
          "Tracking": "Tracking",
          "Type": "Type",
          "Unit Price": "Unit Price",
          "User Name": "User Name",
          "Zip Code": "Zip Code",
          "Failed to change language": "Failed to change language",
          "Change Language": "Change Language",
          "No language options available": "No language options available",
        },
        'zh_CN': {
          "MoreMito": "MoreMito",
          "Success 🎉": "成功 🎉",
          "Thanks for sharing via \$platform": "感谢通过\$platform分享",
          "Thanks for sharing via {platform}": "感谢通过@platform分享",
          "Required": "必须",
          "Please fill in all fields": "请填写所有字段",
          "Enter Contact Details": "输入联系信息",
          "Recipient Name": "收件人姓名",
          "Phone Number": "电话号码",
          "Phone number": "电话号码",
          "Email address": "电子邮件地址",
          "Share Via": "通过以下方式分享",
          "Share Now": "立即分享",
          "Share MoreMito Content": "分享MoreMito内容",
          "Generate Another Link": "生成另一个链接",
          "Step 1: Enter Recipient Name": "第1步：输入收件人姓名",
          "Step 2: Write Message": "第2步：填写消息",
          "Mismatch": "不匹配",
          "New password and confirm password do not match": "新密码和确认密码不匹配",
          "Weak Password": "密码太弱",
          "Password must be at least 6 characters": "密码必须至少6个字符",
          "Success": "成功",
          "Password updated successfully": "密码更新成功",
          "Error": "错误",
          "Something went wrong. Please try again.": "出现问题。请重试。",
          "Permission Required": "需要权限",
          "Please allow contacts permission": "请允许联系人权限",
          "Login Failed": "登录失败",
          "Please check your credentials.": "请检查您的凭证。",
          "Profile updated successfully": "个人资料更新成功",
          "All notifications marked as read": "所有通知标记为已读",
          "Shop Moremito": "购物MoreMito",
          "Incomplete Survey": "未完成的调查",
          "Please answer all questions before submitting.": "提交前请回答所有问题。",
          "Your survey has been submitted.": "您的调查已提交。",
          "Submission failed, try again.": "提交失败，请重试。",
          "Unable to submit survey. Try again later.": "无法提交调查。请稍后重试。",
          "Ticket created successfully.": "工单创建成功。",
          "Ticket reopened": "工单重新打开",
          "Welcome tag updated successfully": "欢迎标签更新成功",
          "My Address Screen": "我的地址屏幕",
          "My Address": "我的地址",
          "Link copied to clipboard": "链接已复制到剪贴板",
          "LINK URL": "链接网址",
          "COPY": "复制",
          "My Deep Links": "我的深层链接",
          "No Deep Links Generated": "未生成深层链接",
          "My Network Screen": "我的网络屏幕",
          "My Network": "我的网络",
          "My Recurring Order Screen": "我的定期订单屏幕",
          "My Recurring Order": "我的定期订单",
          "My Referral Orders Screen": "我的推荐订单屏幕",
          "My Referral Orders": "我的推荐订单",
          "My Personal Referral's Orders": "我的个人推荐订单",
          "My Personal Referral’s Orders": "我的个人推荐订单",
          "Date": "日期",
          "Order No.": "订单号",
          "Rank": "等级",
          "No rank history found": "未找到等级历史",
          "My Rank History": "我的等级历史",
          "Current Rank": "当前等级",
          "Highest Rank": "最高等级",
          "Penny for your thoughts?": "请分享您的想法？",
          "Close": "关闭",
          "Username is required. Please enter your username.":
              "需要用户名。请输入您的用户名。",
          "Password is required. Please enter your password.":
              "密码是必需的。请输入您的密码。",
          "Remember me": "记住我",
          "Enter username": "输入用户名",
          "Enter password": "输入密码",
          "Username*": "用户名*",
          "Password*": "密码*",
          "Login": "登录",
          "To continue, please take a moment to complete a brief survey. Your feedback helps us improve your experience!":
              "要继续，请完成一份短问卷。您的反馈帮助我们改进体验！",
          "Start Survey": "开始调查",
          "Answer Required": "需要答案",
          "Please answer first.": "请先回答。",
          "Select an answer first.": "请先选择答案。",
          "Survey": "调查",
          "MoreMito Library": "MoreMito库",
          "Browse audio, video, and document files.": "浏览音频、视频和文档。",
          "Search Audios, Videos & Docs": "搜索音频、视频和文档",
          "Files": "文件",
          "Categories": "分类",
          "No Contacts Found": "未找到联系人",
          "Search contacts...": "搜索联系人...",
          "Select Contact": "选择联系人",
          "Audio": "音频",
          "Playing audio file.": "正在播放音频文件。",
          "Please enter recipient name": "请输入收件人姓名",
          "Failed to generate link": "生成链接失败",
          "Share": "分享",
          "•  ": "•  ",
          "e.g. John Doe": "例如约翰·多",
          "Write a message for the recipient": "为收件人写一条消息",
          "Enter recipient name": "输入收件人姓名",
          "Message": "消息",
          "Select an Option": "选择选项",
          "Please choose how you would like to share.": "请选择您希望如何分享。",
          "Choose from Contacts": "从联系人中选择",
          "Enter Name Manually": "手动输入名称",
          "Generate Link To Share": "生成分享链接",
          "Generate a Link": "生成链接",
          "Enter the name to track the responses from this person with.":
              "输入姓名以跟踪此人的回复。",
          "Click on \"Generate a Link\" button to create a link that you are going to send to this person.":
              "点击“生成链接”按钮，创建一个将发送给此人的链接。",
          "A personalized message has been created for the recipient.":
              "已为收件人创建个性化消息。",
          "You can review and edit the message if needed.": "如有需要，您可以查看并编辑该消息。",
          "Click Share to send the message to the recipient.":
              "点击“分享”将消息发送给收件人。",
          "Hey {name}, {sender} has shared some information with you. Visit the link to view it. {url}":
              "嘿，@name，@sender与您分享了一些信息。访问此链接查看：@url",
          "Hey {name}, {sender} has invited you to the {title}. Visit the link to see more. {url}":
              "嘿@name，@sender 邀请您参加 @title。访问链接了解更多。@url",
          "Hey {name}, {sender} has invited you to the MoreMito health and wellness call. Visit the link to see more. {url}":
              "嘿@name，@sender 邀请您参加 MoreMito 健康与养生通话。访问链接了解更多。@url",
          "Subcategories": "子类别",
          "No sent history found": "未找到发送历史记录",
          "MoreMito Cash Transfer History": "MoreMito转账历史",
          "Below is the history of the MoreMito Cash you have transferred.":
              "下面是您转账的MoreMito历史。",
          "MoreMito Cash Transferred By You": "您转账的MoreMito",
          "Total Sent: ": "发送总额: ",
          "MoreMito Cash Sent To Others": "发给他人的MoreMito现金",
          "Date Transferred": "转账日期",
          "Sent To": "发送给",
          "Amount": "金额",
          "No transfer history found": "未找到转账历史记录",
          "Below is the history of the MoreMito Cash you have received.":
              "下面是您收到的MoreMito历史。",
          "MoreMito Cash Transferred To You": "转账给您的MoreMito",
          "Total Received: ": "收到总额: ",
          "Transfer History": "转账历史",
          "Date Received": "收到日期",
          "Sent By": "发送者",
          "No payout history found": "未找到支付历史",
          "Requested Commission History": "请求佣金历史",
          "Total Approved Amount": "总批准金额",
          "Commission Payout History": "佣金支付历史",
          "Transaction Id": "交易ID",
          "Payment Method": "付款方式",
          "Payment Status": "付款状态",
          "Description": "描述",
          "Go": "前往",
          "Total Amount": "总金额",
          "Refunded": "已退款",
          "No records found": "未找到记录",
          "Search by Order No.": "按订单号搜索",
          "Compensation Spent On Orders": "订单支出补偿",
          "No orders found": "未找到订单",
          "Order Details": "订单详情",
          "Commission Type Summary": "佣金类型摘要",
          "Commissions Details": "佣金详情",
          "No compensation history found": "未找到补偿历史",
          "Yearly Details": "年度详情",
          "Total Compensation": "总补偿",
          "My Compensation": "我的补偿",
          "MoreMito Cash": "MoreMito现金",
          "MoreMito Commission": "MoreMito佣金",
          "Total Compensation Earned": "获得的总补偿",
          "Average Order Amount": "平均订单金额",
          "Customer Count": "客户数量",
          "Avg. Earned / Customer": "平均赚取/客户",
          "View All Logs": "查看所有日志",
          "Daily Compensation Log": "日补偿日志",
          "No details found": "未找到详情",
          "\$orderId": "\$orderId",
          "Commission processing date": "佣金处理日期",
          "No data available for \$year": "没有\$year的数据可用",
          "No data available for {year}": "@year暂无可用数据",
          "\$year Details": "\$year详情",
          "{year} Details": "@year详情",
          "No content available": "没有可用的内容",
          "Announcement Details": "公告详情",
          "Call Details": "呼叫详情",
          "Welcome": "欢迎",
          "\${user.name} (\${user.userName})":
              "\${user.name} (\${user.userName})",
          "Call Announcement": "呼叫公告",
          "Subject: \${call.subject ?? ": "Subject: \${call.subject ?? ",
          "Click to View": "点击查看",
          "Announcements": "公告",
          "BETA VERSION": "测试版本",
          "v1.0.0": "v1.0.0",
          "This app is currently in Beta, which means some features may be incomplete, under testing, or subject to change. You may experience occasional issues or variations. We appreciate your feedback as we work to improve the app.":
              "此应用目前处于测试版。某些功能可能不完整或正在测试中。感谢您的反馈。",
          "Dashboard": "仪表板",
          "Confirm Exit": "确认退出",
          "Home": "首页",
          "Share Info": "分享信息",
          "Notification": "通知",
          "Setting": "设置",
          "No interactions yet": "暂无互动",
          "Notes saved successfully": "笔记保存成功",
          "\$label:": "\$label:",
          "{label}:": "@label:",
          "Enter internal notes": "输入内部备注",
          "Lead Details": "线索详情",
          "My Leads": "我的潜在客户",
          "Contacted": "已联系",
          "View Details": "查看详情",
          "My Mito Info Shared Links Activity Tracking": "我的Mito信息共享链接跟踪",
          "Track and monitor all your shared links activity.\nView recipient activity and interaction details.":
              "跟踪并监控您所有共享链接的活动。\n查看收件人的活动和互动详情。",
          "Share\nActivity": "分享\n活动",
          "Shared\nby Me": "我分享的",
          "Shared\nwith Me": "与我\n共享",
          "{name}'s Activity": "@name的活动",
          "{nameWithHandle}\nShared Links Activity Tracking":
              "@nameWithHandle\n共享链接活动跟踪",
          "View shared links activity report shared with you.\nView recipient activity and interaction details.":
              "查看与您共享的链接活动报告。\n查看收件人的活动和互动详情。",
          "No activity data available.\nPull down to refresh.":
              "暂无活动数据。\n下拉刷新。",
          "Last 72 Hours": "最近72小时",
          "Last 7 Days": "最近7天",
          "Last 2 weeks": "最近2周",
          "Last 3 weeks": "最近3周",
          "Last 4 weeks": "最近4周",
          "Lifetime": "全部时间",
          "People Health\nProducts": "人类健康\n产品",
          "Pet Health\nProducts": "宠物健康\n产品",
          "Buy Now": "立即购买",
          "All": "全部",
          "Audios, Videos & Docs": "音频、视频和文档",
          "Flyers": "传单",
          "SMS requested info ": "短信请求信息 ",
          "Total Activities : \${item.totalInteractions ?? 0}":
              "总活动：\${item.totalInteractions ?? 0}",
          "Total Activities : {total}": "总活动：@total",
          "Search recipient": "搜索收件人",
          "Shared Flyers": "共享传单",
          "Who :": "谁：",
          "What :": "什么：",
          "When :": "何时：",
          "Last Activity :": "最后活动：",
          "Activity Tracking Summary (all links)": "活动跟踪摘要（所有链接）",
          "These cards show total recipients and total activity ":
              "这些卡片显示总收件人和总活动 ",
          "Retry": "重试",
          "\${widget.userName}": "\${widget.userName}",
          "{name} ({username})": "@name (@username)",
          "Remove Share": "移除共享",
          "Cancel": "取消",
          "Remove": "删除",
          "Users I Have Shared Reports With": "我已共享报告的用户",
          "Manage users who can see your shared link activity report.":
              "管理能查看您共享链接报告的用户。",
          "You haven": "您没有",
          "My Shared Reports": "我的共享报告",
          "Decline Report": "拒绝报告",
          "Decline report shared by ": "拒绝由以下用户共享的报告 ",
          "Decline": "拒绝",
          "View": "查看",
          "Activities Shared With Me": "与我共享的活动",
          "Reports shared with you by other users.": "其他用户与您共享的报告。",
          "No activities shared with you yet.": "目前没有与您共享的活动。",
          "Reason (optional)": "原因（可选）",
          "Shared With Me": "与我共享",
          "Share Your Activity": "分享您的活动",
          "Search": "搜索",
          "Enter a username and tap Search.": "输入用户名并点击搜索。",
          "\${results.length} result(s)": "\${results.length}个结果",
          "{count} result(s)": "@count个结果",
          "@\${user.username ?? ": "@\${user.username ?? ",
          "Choose": "选择",
          "Share Report": "分享报告",
          "Review the selected user and optionally add a note before sharing.":
              "在分享前查看选定的用户，可选择添加备注。",
          "Selected User Details": "选定用户的详情",
          "Note (Optional):": "备注（可选）：",
          "Different User": "不同用户",
          "Enter a note (max 500 characters)": "输入备注（最多500个字符）",
          "Search by Username": "按用户名搜索",
          "Allow Others to Request MoreMito Info from Me": "允许他人向我请求MoreMito信息",
          "Notes": "笔记",
          "TMRIS Leads": "TMRIS潜在客户",
          "-": "-",
          "TMRIS Lead Details": "TMRIS潜在客户详情",
          "Order Details for Order No. ": "订单\${orderId}的订单详情 ",
          "\${address.address1}, \${address.city}, \${address.stateName}, \${address.countryName} - \${address.zip}":
              "\${address.address1}, \${address.city}, \${address.stateName}, \${address.countryName} - \${address.zip}",
          "Product(s)": "产品",
          "Quantity: \${p.quantity}": "数量：\${p.quantity}",
          "Quantity: {quantity}": "数量：@quantity",
          "Notification Details": "通知详情",
          "Notifications": "通知",
          "Mark all read": "标记全部已读",
          "Push Notifications": "推送通知",
          "Manage how you receive alerts and updates": "管理您如何接收警报和更新",
          "Enable/Disable app push notifications": "启用或禁用应用通知",
          "All Notifications": "所有通知",
          "Shared Link Notifications": "共享链接通知",
          "Messages about promotions and information sent by us":
              "我们发送的关于促销和信息",
          "Announcement Notifications": "公告通知",
          "Important updates and reminders": "重要更新和提醒",
          "Reset": "重置",
          "Downline Orders": "下线订单",
          "No order details found": "未找到订单详情",
          "Order #\${data.myOrders.orderId}": "订单 #\${data.myOrders.orderId}",
          "Order #{orderId}": "订单 #@orderId",
          "Products": "产品",
          "Downline Order Details": "下线订单详情",
          "Order Number : \${order.orderId}": "订单号：\${order.orderId}",
          "Order Number : {orderId}": "订单号：@orderId",
          "Shipping Method": "运费方式",
          "Tracking ID": "追踪ID",
          "Send/View messages with support team": "与支持团队发送和查看消息",
          "My Order": "我的订单",
          "Orders": "订单",
          "Referral & Downline Orders": "推荐和下线订单",
          "Billing Address": "账单地址",
          "Order Summary": "订单摘要",
          "Order Number - \${_value(info?.customOrderNumber)}":
              "订单号 - \${_value(info?.customOrderNumber)}",
          "Order Number - {orderNumber}": "订单号 - @orderNumber",
          "Order Date: \$date (UTC)": "订单日期：\$date (UTC)",
          "Order Date: {date} (UTC)": "订单日期：@date (UTC)",
          "Shipping Address": "送货地址",
          "Please select Country and State": "请选择国家和州",
          "Set as primary address": "设为主要地址",
          "Select \$label": "选择\$label",
          "Select {label}": "选择@label",
          "Contact Person": "联系人",
          "Address": "地址",
          "Country": "国家/地区",
          "State": "州",
          "Create new password": "创建新密码",
          "Your new password must be different from previously used passwords.":
              "新密码必须与以前使用的密码不同。",
          "Please Enter \$label": "请输入\$label",
          "Please Enter {label}": "请输入@label",
          "Change Password": "更改密码",
          "Update Password": "更新密码",
          "Current Password": "当前密码",
          "New Password": "新密码",
          "Confirm Password": "确认密码",
          "Note: Changing your address here does not update existing recurring orders. Please update those manually in the Orders tab.":
              "注意：更改地址不会更新现有定期订单。请在订单选项卡中手动更新。",
          "\${address.firstName} \${address.lastName}":
              "\${address.firstName} \${address.lastName}",
          "Default": "默认",
          "No addresses yet": "暂无地址",
          "Add your delivery address to get started": "添加送货地址以开始",
          "Manage Addresses": "管理地址",
          "Edit": "编辑",
          "Set as Default": "设为默认",
          "Add New Address": "添加新地址",
          "Note: Changing your address here does not change your address on already created recurring orders.":
              "注意：更改地址不会影响已创建的定期订单。",
          "No addresses found.": "未找到地址。",
          "My Profile": "我的个人资料",
          "Account Information": "账户信息",
          "Username": "用户名",
          "SMS Code": "短信代码",
          "First Name": "名",
          "Last Name": "姓",
          "Phone": "电话",
          "WhatsApp Phone": "WhatsApp手机号",
          "Membership Type": "会员类型",
          "Join Date": "加入日期",
          "City": "城市",
          "Zip": "邮编",
          "Name": "姓名",
          "Update Profile": "更新个人资料",
          "Manage your addresses": "管理您的地址",
          "Edit address": "编辑地址",
          "Your Website Header Display": "您的网站标题显示",
          "Loading...": "正在加载...",
          "User Role": "用户角色",
          "Change Role": "更改角色",
          "Use aliases (my Welcome Tag info) for name, email, and phone number":
              "使用别名(我的欢迎标签信息)作为名称、电子邮件和电话号码",
          "Save Changes": "保存更改",
          "Grief Relief Zoom Call Screen": "悲伤救济Zoom通话屏幕",
          "Grief Relief Zoom Call": "悲伤救济Zoom通话",
          "My Info": "我的信息",
          "My Account Settings": "我的账户设置",
          "My Personals": "我的个人信息",
          "My Support Network": "我的支持网络",
          "Email Notification Settings": "电子邮件通知设置",
          "Text Notification Settings": "短信通知设置",
          "User Survey": "用户调查",
          "My Tree View": "我的树状图",
          "My Star Tree View": "我的星级树状图",
          "Search My Network": "搜索我的网络",
          "Shop MoreMito Products": "购买MoreMito产品",
          "My Orders": "我的订单",
          "My Personal Referral": "我的个人推荐",
          "Create A New Autoship Order": "创建新的自动发货订单",
          "Recurring Orders": "重复订单",
          "Payment Settings": "支付设置",
          "Compensation": "补偿",
          "My Compensations": "我的补偿",
          "My Daily Compensation Log": "我的每日补偿日志",
          "My Compensation History": "我的补偿历史",
          "Request Payouts And Make Transfers": "请求支付并进行转账",
          "Request A Payout": "请求支付",
          "History Of Compensation Spent": "补偿支出历史",
          "My Currently Available Balance": "我的当前可用余额",
          "Manage My Payout Methods": "管理我的付款方式",
          "The Compensation Plan": "补偿计划",
          "My Rank Management": "我的排名管理",
          "Rank Management Report": "排名管理报告",
          "Resources": "资源",
          "Direct Links": "直接链接",
          "Grief Relief Zoom Call 7-14-24": "悲伤救济 Zoom 通话 7-14-24",
          "Health & Wellness Call Replays": "健康与养生通话回放",
          "Share MoreMito Info": "分享 MoreMito 信息",
          "Customize And Share My Flyers": "自定义并分享我的传单",
          "My Flyers": "我的传单",
          "Share Audios, Videos & Docs Files": "分享音频、视频和文档",
          "Allow Others To Request MoreMito Info From Me": "允许他人向我请求MoreMito信息",
          "See My Generated Leads": "查看我生成的潜在客户",
          "See & Track Activity From My Shared Links": "查看和跟踪我共享链接的活动",
          "Support": "支持",
          "Support Ticket List": "支持工单列表",
          "Create Support Ticket": "创建支持工单",
          "Order Comments": "订单评论",
          "Transfer MoreMito Cash": "转账 MoreMito 现金",
          "Menu": "菜单",
          "Mitochondria Story Screen": "线粒体故事",
          "Mitochondria Story": "线粒体故事",
          "Settings": "设置",
          "Account": "账户",
          "Actions": "操作",
          "Logout": "注销",
          "Confirm Logout": "确认注销",
          "Testimonials Screen": "推荐证明屏幕",
          "Testimonials": "推荐证明",
          "Exciting features are on the way! Stay tuned for updates and new enhancements.":
              "令人兴奋的功能即将推出！敬请关注更新。",
          "Back to Home": "返回首页",
          "Warning": "警告",
          "Please fill all fields": "请填写所有字段",
          "Enter subject": "输入主题",
          "Select": "选择",
          "Describe your issue (order no., username, etc.)": "描述您的问题(订单号、用户名等)",
          "Ticket Subject:": "工单主题：",
          "Ticket Related To:": "工单相关：",
          "Ticket Priority:": "工单优先级：",
          "Ticket Description:": "工单描述：",
          "Create Ticket": "创建工单",
          "#\${ticket.ticketId}": "#\${ticket.ticketId}",
          "Priority": "优先级",
          "Subject": "主题",
          "Re-open Ticket": "重新打开工单",
          "Support Tickets": "支持工单",
          "Enter comment": "输入评论",
          "Previous Comments": "之前的评论",
          "No comments yet": "暂无评论",
          "Add Comment": "添加评论",
          "\${c.firstName ?? ": "\${c.firstName ?? ",
          "Write your comment...": "写你的评论...",
          "Ticket #\${widget.ticketId}": "工单 #\${widget.ticketId}",
          "Ticket #{ticketId}": "工单 #@ticketId",
          "Submit Comment": "提交评论",
          "Invalid username or password.": "用户名或密码无效。",
          "Access Denied!": "被拒绝访问！",
          "Session expired. Please log in again.": "会话已过期。请重新登录。",
          "This update is mandatory": "此更新是强制性的",
          "Update Now": "现在更新",
          "Connection Error: \$e": "连接错误：\$e",
          "Connection Error: {error}": "连接错误：@error",
          "You": "您",
          "No Share 😕": "未共享😕",
          "Looks like the share wasn’t completed. No worries! You can try again whenever you":
              "看起来分享未完成。不用担心！您可以随时重试",
          "Looks like the share wasn’t completed. No worries! You can try again whenever you're ready. We appreciate you! ❤️":
              "看起来分享未完成。不用担心！您准备好后可随时再试。感谢您的支持！❤️",
          "You're amazing! 🌟 Thank you for sharing on \$platform. We truly appreciate your support! 🙌😊":
              "您太棒了！🌟 感谢您在\$platform上分享。我们非常感谢您的支持！🙌😊",
          "You're amazing! 🌟 Thank you for sharing on {platform}. We truly appreciate your support! 🙌😊":
              "您太棒了！🌟 感谢您在@platform上分享。我们非常感谢您的支持！🙌😊",
          "WhatsApp": "WhatsApp",
          "Email": "电子邮件",
          "More": "更多",
          "Test": "测试",
          "⚠️ iOS token fetch error: \$e": "⚠️ iOS令牌获取错误：\$e",
          "⚠️ iOS token fetch error: {error}": "⚠️ iOS令牌获取错误：@error",
          "Check your internet connection.": "检查您的互联网连接。",
          "Try again in a few moments.": "稍后重试。",
          "Please grant \$permission permission in the settings.":
              "请在设置中授予\$permission权限。",
          "Please grant {permission} permission in the settings.":
              "请在设置中授予@permission权限。",
          "Open Settings": "打开设置",
          "Lost Connection": "连接丢失",
          "Preview Document": "预览文档",
          "Open in Browser": "在浏览器中打开",
          "Token copied to clipboard!": "令牌已复制到剪贴板！",
          "FCM Token": "FCM令牌",
          "Your Device Token:": "您的设备令牌：",
          "Copy Token": "复制令牌",
          "No internet connection": "没有互联网连接",
          "Please check your keyword or try again your browsing keyword":
              "请检查您的关键字或重试您的浏览关键字",
          "Check your network": "检查您的网络",
          "Try again": "重试",
          "Oops! No \${widget.title ?? ": "哎呀！没有\${widget.title ?? ",
          "Loading": "加载中",
          "Preparing page...": "页面准备中...",
          "Video failed to load": "视频加载失败",
          "Error loading video": "加载视频发生错误",
          "Language": "语言",
          "English": "English",
          "Chinese": "中文",
          "Activity": "活动",
          "Address Line 1": "地址行1",
          "Address Line 2": "地址行2",
          "Amount Paid": "已支付金额",
          "Average Amount": "平均金额",
          "Avg Earned / Customer": "平均收益/客户",
          "Call Me": "给我打电话",
          "Commission Level": "佣金级别",
          "Commission Type": "佣金类型",
          "Compensation %": "补偿百分比",
          "Compensation Amount": "补偿金额",
          "Compensation Type": "补偿类型",
          "Contacted Date": "联系日期",
          "Customer": "客户",
          "Email Address": "电子邮件地址",
          "Gross Order Amount": "订单总金额",
          "Interest": "兴趣",
          "Item Total": "小计",
          "Level": "等级",
          "Order Amount": "订单金额",
          "Order Count": "订单数量",
          "Order Date": "订单日期",
          "Order No": "订单号",
          "Order Total": "订单总计",
          "OrderId": "订单ID",
          "Payment": "付款",
          "Price": "价格",
          "Product": "产品",
          "Qty": "数量",
          "Quantity": "数量",
          "Recipients": "收件人",
          "Running Total": "累计总计",
          "Sales Tax": "销售税",
          "Shipping": "运费",
          "Shipping Amount": "运费金额",
          "Shipping Fee": "运费",
          "Status": "状态",
          "Street Address": "街道地址",
          "Sub Total": "小计",
          "Submitted": "已提交",
          "Tax": "税费",
          "Total": "总计",
          "Total Earned": "总收益",
          "Tracking": "追踪",
          "Type": "类型",
          "Unit Price": "单价",
          "User Name": "用户名",
          "Zip Code": "邮编",
          "Change Language": "更改语言",
          "Language Settings": "语言设置",
          "No language options available": "暂无可用语言选项",
          "Language changed to English": "语言已更改为英文",
          "Language changed to Chinese": "语言已更改为中文",
          "Failed to change language": "更改语言失败",
          "Help & Support": "帮助和支持",
        },
      };
}
